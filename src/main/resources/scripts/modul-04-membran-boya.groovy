...
def devam = waitForConfirm(
    ...
    "  • 1+ (zayıf):       0.15 OD\n" +
    "  • 2+ (orta):        0.40 OD  ← araştırma/eğitim eşiği\n" +
    "  • 3+ (güçlü):       0.70 OD  ← araştırma/eğitim eşiği\n\n" +
...
// H-score membran/sitoplazmik markerlar için araştırma/eğitim amaçlı özet metriktir.
def hScore = pct1 + 2.0 * pct2 + 3.0 * pct3

// En yüksek yoğunluk binlerinin yayılımına dayalı araştırma/eğitim özeti.
def intensityBand
if (pct3 >= 10) {
    intensityBand = String.format("3+ bin yayılımı yüksek (%%%.1f hücre 3+ binde)", pct3).replace("%%", "%")
} else if (pct2 + pct3 >= 10) {
    intensityBand = String.format("2+/3+ bin yayılımı belirgin (%%%.1f hücre 2+ veya 3+ binde)", pct2 + pct3).replace("%%", "%")
} else if (pct1 + pct2 + pct3 >= 10) {
    intensityBand = String.format("Düşük yoğunluk dağılımı (toplam pozitif %%%.1f, hiçbir bin >%%10 değil)", pct1 + pct2 + pct3).replace("%%", "%")
} else {
    intensityBand = String.format("Pozitif bin yayılımı düşük (toplam pozitif %%%.1f, ≥%%10 eşiği aşılmadı)", pct1 + pct2 + pct3).replace("%%", "%")
}
...
showResultWindow(
    ...
    String.format(
        ...
        "────────────────────────\n" +
        "  H-score (0–300)     : %.0f       ← araştırma/eğitim özeti\n" +
        "  Süre                : %.1f sn\n" +
        "%s\n" +
        "🧪 Yoğunluk dağılımı özeti:\n" +
        "    %s\n\n" +
...
println "  H-score: ${String.format('%.0f', hScore)}  |  Yoğunluk özeti: ${intensityBand}"
println "─────────────────────────────────────"
