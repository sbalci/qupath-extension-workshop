...
// ASCO/CAP-style band — UYARI: bu yalnızca toplam yüzde eşiklerini gösteren bir
// etikettir. ASCO/CAP 2018 gerçek skorlaması membran tutulumunun **tamlığı +
// yoğunluğu + dağılımı**'na bakar (örn. 1+ = "%10'dan fazla hücrede zayıf,
// kesik membran"; 2+ = "%10'dan fazla hücrede zayıf-orta, tam membran";
// 3+ = "%10'dan fazla hücrede yoğun, tam, uniform membran"). Computational
// çıktı bu morfolojik kriterleri ölçemez — sadece DAB OD yoğunluğunu sayar.
// Aşağıdaki band, en yüksek skorlu binin yayılımına dayalı, yorum gerektiren
// bir özetidir; klinik karar değildir.
def ascoCapBand
if (pct3 >= 10) {
    ascoCapBand = String.format("3+ aralığında  (%%%.1f hücre 3+ binde, yayın eşiği ≥%%10)", pct3).replace("%%", "%")
} else if (pct2 + pct3 >= 10) {
    ascoCapBand = String.format("2+ / equivocal aralığında  (%%%.1f hücre 2+ veya 3+ binde)", pct2 + pct3).replace("%%", "%")
} else if (pct1 + pct2 + pct3 >= 10) {
    ascoCapBand = String.format("Düşük pozitivite — 1+ veya weak  (toplam pozitif %%%.1f, hiçbir bin >%%10 değil)", pct1 + pct2 + pct3).replace("%%", "%")
} else {
    ascoCapBand = String.format("0 aralığında  (toplam pozitif %%%.1f, ≥%%10 eşiği aşılmadı)", pct1 + pct2 + pct3).replace("%%", "%")
}
...
showResultWindow(
    ...
    String.format(
        ...
        "  Süre                : %.1f sn\n" +
        "%s\n" +
        "🩺 ASCO/CAP yüzde aralığı (yalnızca yayılım — tamamlık/uniformluk değil):\n" +
        "    %s\n\n" +
        "ÖNEMLİ: Computational yüzde sadece yayılımı ölçer; ASCO/CAP 2018 gerçek\n" +
        "skoru membranın **tamlığı, uniformluğu ve yoğunluğu**'na bakar (bu yazılım\n" +
        "ölçemez). Klinik karar (Pozitif / Equivocal → FISH / Negatif) için patolog\n" +
        "görsel skoru zorunludur — computational yalnızca tarama amaçlı.\n\n" +
...
println "  H-score: ${String.format('%.0f', hScore)}  |  ASCO/CAP aralığı: ${ascoCapBand}"
println "─────────────────────────────────────"
