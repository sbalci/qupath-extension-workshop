...
// 2) Karşılama
// ──────────────────────────────────────────────────────────────
def cyto1 = atolyeD('atolye.cyto1', 0.10)
def cyto2 = atolyeD('atolye.cyto2', 0.20)
def cyto3 = atolyeD('atolye.cyto3', 0.35)
...
def cellExpansionCyto = atolyeD('atolye.cellExpansionCyto', 7.0)
def medianRadius = atolyeD('atolye.medianRadius', 0.0)
def watershed = atolyeB('atolye.watershed', true)
...
def devam = waitForConfirm(
    ...
    "  • 1+ (zayıf):       ${cyto1}${cyto1 != 0.10 ? ' (değiştirildi)' : ''} OD  ← nükleerden düşük (sitoplazma büyük hacim)\n" +
    "  • 2+ (orta):        ${cyto2}${cyto2 != 0.20 ? ' (değiştirildi)' : ''} OD\n" +
    "  • 3+ (güçlü):       ${cyto3}${cyto3 != 0.35 ? ' (değiştirildi)' : ''} OD\n\n" +
    "Hücre genişletme (cell expansion): **${cellExpansionCyto}${cellExpansionCyto != 7.0 ? ' (değiştirildi)' : ''} µm** (sitoplazma için daha büyük örnekleme halkası)\n\n" +
    "Çıktı:\n" +
...
println "  • Eşik 1+ / 2+ / 3+: ${cyto1} / ${cyto2} / ${cyto3} OD"
println "  • Hücre genişletme (cell expansion): ${cellExpansionCyto} µm"
