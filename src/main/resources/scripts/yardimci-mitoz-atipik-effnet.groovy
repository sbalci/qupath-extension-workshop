/**
 * Modül - Atipik mitoz sınıflama (MIDOG25 Track-2 EfficientNetV2, tespit SONRASI)
 * ------------------------------------------------------------------------------
 ...
 *
 * AĞIRLIK LİSANSI:
 *   efficientnetv2_m_fold3_best.pth çalışma anında v1.0.0 yayınından indirilir; paketlenmez.
 *   Kaynak deponun LİSANS dosyası yok → araştırma/eğitim, kullanıcı sorumluluğunda.
 *   Atıf: MIDOG 2025, DeepMicroscopy.
 *
 * ÇALIŞMA ZAMANI:
 *   torch + timm ortamı (env id: midog-atypical) + midog/atypical_effnet.py köprüsü.
 *   Kurulum: Extensions → Atölye → Yardımcılar → Python köprüleri → Atölye Python ortam
 *   yöneticisi → "MIDOG25 EffNetV2 — atipik sınıflandırıcı".
 *
...
def MODEL          = 'EfficientNetV2-M (MIDOG25 Track-2)'
def ENV_ID         = 'midog-atypical'
def WEIGHTS_NAME   = 'efficientnetv2_m_fold3_best.pth'
...
def prefs = java.util.prefs.Preferences.userRoot().node('/qupath/atolye/mitoz-atipik-effnet')
...
def modelCacheFile = { -> new File(new File(new File(atolyeDataRoot(), 'cache'), 'midog-atypical'), WEIGHTS_NAME) }
...
def detectRunner = { ->
    ...
    for (r in roots) { def f = new File(r, 'python/midog/atypical_effnet.py'); if (f.isFile()) return f.getAbsolutePath() }
    return ''
}
def loadConfig = { ->
...
def configMissing = { cfg ->
    def miss = []
    if (!cfg.python?.trim() || !(new File(cfg.python)).isFile()) miss << 'Python yürütülebilir (midog-atypical venv)'
    if (!cfg.runner?.trim() || !(new File(cfg.runner)).isFile()) miss << 'Köprü betiği (atypical_effnet.py)'
    return miss
}
...
if (isHeadless) {
    ...
    println "Atipik sınıflama (EffNetV2): python=${cfg.python ?: '(ayarsız)'} runner=${cfg.runner ?: '(ayarsız)'}"
        ...
        try {
            ...
            javafx.application.Platform.runLater {
                try {
                    ...
                    sb << "ATİPİK MİTOZ SINIFLAMA (EfficientNetV2)\n═══════════════════════════════════════\n\n"
...
render = { ->
    ...
    if (cur == 'CONFIG_INCOMPLETE') {
        ...
        qupath.fx.utils.GridPaneUtils.addGridRow(grid, row++, 0, null, new javafx.scene.control.Label('Köprü (atypical_effnet.py):'), rnField, navButton('…', { browseFile(rnField) }))
        ...
        def mcLbl = new javafx.scene.control.Label(mcf.isFile() ? ('✓ Yerel model VAR: ' + mcf.getAbsolutePath()) : '○ Yerel model yok — "Modeli yerel indir" ile bir kez indirin (~213 MB).')
        ...
        if (imageData == null) {
            ...
            title.setText('Atipik mitoz sınıflama — EfficientNetV2')
            def sb = new StringBuilder()
            sb << "Slayt          : " << imageNameOf(imageData) << "\n"
            sb << "Model          : " << MODEL << "\n"
...
javafx.application.Platform.runLater {
    try {
        ...
        stage.setTitle('Atipik mitoz sınıflama — EfficientNetV2 (MIDOG25 Track-2)'); stage.setAlwaysOnTop(alwaysTop.get())
        render(); stage.show()
    } catch (Throwable t) { Dialogs.showErrorMessage('Sihirbaz açılamadı', t.getClass().getSimpleName() + ': ' + (t.getMessage() ?: '')) }
}
println "✓ Atipik mitoz sınıflama (EfficientNetV2 · MIDOG25 Track-2) sihirbazı açıldı."
