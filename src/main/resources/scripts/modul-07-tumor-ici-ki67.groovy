 ...
 * -----------------------------------------------------------
 * Bu script atölyenin **translasyonel anahtar adımıdır**: önce piksel
 ...
 *     • Gerçek değerden DÜŞÜK Ki-67 LI raporlanır
 *     • Meme luminal A vs B, NET derecesi gibi klinik kararlar etkilenir
 *
 *   Computational analiz tümör/stroma ayrımını AÇIK BİR ADIM olarak
 *   yapmak zorundadır — patolog gözüyle bu doğal olarak yapılır.
 *
 * KULLANIM:
 ...
 *   3. Ki-67 LI'yi yalnızca tümör hücrelerinde rapor et
 */
...
// Önceki Tumor/Stroma anotasyonları varsa temizle
...
if (!existing.isEmpty()) {
    ...
    println "  Önceki ${existing.size()} segmentasyon nesnesi temizlendi."
}
...
def tumorCells = []
tumorAnnotations.each { ann -> tumorCells.addAll(ann.getChildObjects()) }
...
showResultWindow(
    ...
    String.format(
        ...
        "Tümör/stroma sınırının görsel olarak doğrulanması ve eşik seçimi patolog\n" +
        "kararıdır; bu çıktı yalnızca araştırma/yardımcı tarama amaçlıdır.\n\n" +
        "📝 Raporlama notu (öneri):\n" +
        "  \"Ki-67 LI: %%%.1f (yalnızca tümör alanı, n=%,d hücre).\n" +
        "   Yöntem: QuPath 0.6 + RF piksel sınıflandırıcı + Positive cell detection.\"\n\n" +
        "Modül 8'de yayınlanmış bir pipeline'ı (QuANTUM) deneyeceğiz.",
        ...
        tumorStats.ki67LI, density, totalElapsed,
